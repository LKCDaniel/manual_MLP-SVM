...
class MLP:
    ...
    def train(self):
        ...
        print(f'Cross-validation results:\naverage epochs: {np.mean(epochs):.0f} ± {np.std(epochs):.0f}, '
...
def main():
    ...
    datas, labels = [], []
    with open(os.path.join('Iris DataSets', 'iris.data'), 'r') as f:
        for line in f:
            if line == "\n" or line == "":
                break
            line = line.strip().split(',')
            datas.append(list(map(float, line[:4])))
            labels.append(mapping[line[4]])

    dataloader = Dataloader(datas, labels, fold=5)

    mlp = MLP(dataloader, hidden_size=256, regularization=None, fold=5, lr=0.01, max_epoch=5000, early=300, gamma=0.999)
    mlp.train()

    mlp = MLP(dataloader, hidden_size=256, regularization='L1', lambda_reg=0.01, fold=5, lr=0.01, max_epoch=5000,
              early=300, gamma=0.999)
    mlp.train()

    mlp = MLP(dataloader, hidden_size=256, regularization='L2', lambda_reg=0.01, fold=5, lr=0.01, max_epoch=5000,
              early=300, gamma=0.999)
    mlp.train()

    mlp = MLP(dataloader, hidden_size=256, regularization='L1', lambda_reg=0.1, fold=5, lr=0.01, max_epoch=5000,
              early=300, gamma=0.999)
    mlp.train()

    mlp = MLP(dataloader, hidden_size=256, regularization='L2', lambda_reg=0.1, fold=5, lr=0.01, max_epoch=5000,
              early=300, gamma=0.999)
    mlp.train()


    mlp = MLP(dataloader, hidden_size=128, fold=5, lr=0.01, max_epoch=5000, early=1000, gamma=0.999)
    mlp.train()

    mlp = MLP(dataloader, hidden_size=64, fold=5, lr=0.01, max_epoch=5000, early=1000, gamma=0.999)
    mlp.train()

    mlp = MLP(dataloader, hidden_size=48, fold=5, lr=0.01, max_epoch=5000, early=1000, gamma=0.999)
    mlp.train()

    mlp = MLP(dataloader, hidden_size=32, fold=5, lr=0.01, max_epoch=5000, early=1000, gamma=0.999)
    mlp.train()

    mlp = MLP(dataloader, hidden_size=16, fold=5, lr=0.01, max_epoch=5000, early=1000, gamma=0.999)
    mlp.train()

    mlp = MLP(dataloader, hidden_size=8, fold=5, lr=0.01, max_epoch=5000, early=1000, gamma=0.999)
    mlp.train()
